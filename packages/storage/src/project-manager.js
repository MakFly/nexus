...
export class ProjectManager {
    ...
    async detectProject(path) {
        ...
        let project = this.globalDb.queryOne(`
    ...
    `, path);
        ...
        for (let i = parts.length; i > 0; i--) {
            ...
            project = this.globalDb.queryOne(`
        SELECT * FROM projects WHERE root_path = ?
      `, parentPath);
            if (project)
                return project;
    ...
    async getProject(name) {
        if (!this.globalDb)
            await this.init();
        const project = this.globalDb.queryOne(`
    ...
    async updateProjectStats(name) {
        ...
        const fileCount = projectDb.queryOne('SELECT COUNT(*) as count FROM files');
        const chunkCount = projectDb.queryOne('SELECT COUNT(*) as count FROM chunks');
        const memoryCount = projectDb.queryOne('SELECT COUNT(*) as count FROM observations');
        const patternCount = projectDb.queryOne('SELECT COUNT(*) as count FROM patterns');
        if (!this.globalDb)
            await this.init();
        this.globalDb.run(`
      UPDATE projects
      ...
      WHERE name = ?
    `, fileCount?.count || 0, chunkCount?.count || 0, memoryCount?.count || 0, patternCount?.count || 0, name);
    }
    /**
     * Promote pattern to global
     */
    async promotePattern(patternId, fromProject) {
        if (!this.globalDb)
            await this.init();
    ...
    `, patternId);
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `, pattern.intent, pattern.title, pattern.tags_json, pattern.templates_json, pattern.usage_count, pattern.success_count, pattern.fail_count, fromProject);
        // Remove from project
