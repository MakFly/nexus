...
export interface RgSearchOptions {
  ...
  ignoreCase?: boolean;     // default: true (smart case)
  regex?: boolean;          // treat query as regex
  wordBoundary?: boolean;   // match whole words only
}
 ...
 */
export async function rgSearch(options: RgSearchOptions): Promise<RgSearchResult> {
  const {
    query,
    path,
    maxResults = 20,
 ...
 */
function parseRgOutput(output: string, maxResults: number): RgSearchResult {
  ...
  const matchesByFile = new Map<string, any[]>();
...
export async function searchByType(
  query: string,
  path: string,
  fileType: 'ts' | 'js' | 'py' | 'go' | 'rust' | 'java'
): Promise<RgMatch[]> {
 ...
 */
export function formatMatch(match: RgMatch): string {
 ...
 */
export function formatMatchList(matches: RgMatch[]): string {
