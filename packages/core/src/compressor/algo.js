 ...
 */
function extractSymbols(text, maxSymbols = 15) {
    const patterns = [
        // File paths (src/lib/...)
    ...
    if (parsed.files && Array.isArray(parsed.files)) {
        parsed.files.forEach((f) => symbols.add(f));
    }
 ...
 */
function extractNumbers(text, maxNumbers = 5) {
    const numbers = [];
 ...
 * ```
 */
export function compressAlgorithmic(content, maxTokens = 50) {
    // Parse content
 ...
 */
export function estimateAlgorithmicRatio(content) {
